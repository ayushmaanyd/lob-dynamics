...
public:
    ...
    // Trade observer: invoked on every execution with (executionPrice, sharesFilled,
    // aggressorBuy). Set externally by a recorder; left null in normal operation so the
    // engine's behaviour is unchanged when unused.
    std::function<void(int, int, bool)> onTrade;
