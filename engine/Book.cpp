...
void Book::marketOrderHelper(int orderId, bool buyOrSell, int shares)
{
    ...
    while (bookEdge != nullptr && bookEdge->getHeadOrder()->getShares() <= shares)
    {
        ...
        // Record the execution before mutating the book: resting limit price is the
        // execution price; the whole resting order is consumed here.
        if (onTrade) onTrade(bookEdge->getLimitPrice(), headOrder->getShares(), buyOrSell);
    ...
    {
        // Partial fill of the resting head order at its limit price.
        if (onTrade) onTrade(bookEdge->getLimitPrice(), shares, buyOrSell);
