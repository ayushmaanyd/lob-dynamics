...
# Approximate queue-position model (optional)
try:
...
def _fill_at_quote(qrow: pd.Series, side: str, qty: float, cost: CostModel, force_taker=True) -> Tuple[float,float,bool]:
    """
    Return (filled_qty, exec_px, taker?). We intentionally *always* fill the desired
    qty (bounded by L1 size when available) so each bar shows non-zero fills.
    """
    if qty <= 0:
...
def _run_bar_loop_with_queue(
    ...
    """
    Universal loop for TWAP/VWAP/POV, with a safety fill guard.
...
def main():
    ap = argparse.ArgumentParser(prog="lobkit backtest", description="Strategy backtester (TWAP/VWAP/POV).")
    ap.add_argument("--strategy", required=True, help="YAML file (see configs/*.yaml)")
    ap.add_argument("--quotes",   required=False, help="Quotes CSV from the engine recorder")
